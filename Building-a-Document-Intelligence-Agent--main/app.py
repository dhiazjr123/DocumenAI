...
with col2:
    ...
    if st.session_state.documents:
        ...
        with st.expander("📋 Document List"):
            ...
            # Integrated Debug Info section (within Document List)
            if st.session_state.get('show_debug', False):
                ...
                _img_state = st.session_state.get('image_bytes', {})
                _img_keys = list(_img_state.keys()) if isinstance(_img_state, dict) else (["<single_image>"] if _img_state else [])
                st.markdown(f"- Image bytes keys: {_img_keys}")
        ...
        # Debug section removed; now integrated in Document List
        ...
        if st.session_state.get("clear_qa_input_flag", False):
            st.session_state["qa_input"] = ""
            st.session_state.clear_qa_input_flag = False

        user_q = st.text_input("Your question (can ask about specific documents or compare them):", key="qa_input")
        if st.button("Ask") and st.session_state.get("qa_input"):
            q = st.session_state.qa_input
            st.session_state.chat_history.append({"role": "user", "content": q})
            with st.spinner("Generating response..."):
                if not google_api_key:
                    ans = "Please provide a valid Google API Key."
                else:
                    ans = generate_response_with_fallback(st.session_state.ocr_content, q)
            st.session_state.chat_history.append({"role": "assistant", "content": ans})
            st.session_state.clear_qa_input_flag = True
            st.rerun()
    else:
        ...
        user_q = st.text_input("Your question (you can also paste a URL first):", key="qa_input")
        if st.button("Ask") and st.session_state.get("qa_input"):
